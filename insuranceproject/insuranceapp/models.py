...
class Insurace(models.Model):
    ...
    def get_create_url(cls):
        return f"insuranceapp:{cls.get_url_name_prefix()}-create"
    ...
    def get_detail_url(self):
        return reverse(
            f"insuranceapp:{self.__class__.get_url_name_prefix()}-detail",
            kwargs={"pk": self.id},
        )

    def get_update_url(self):
        return reverse(
            f"insuranceapp:{self.__class__.get_url_name_prefix()}-update",
            kwargs={"pk": self.id},
        )

    def get_delete_url(self):
        return reverse(
            f"insuranceapp:{self.__class__.get_url_name_prefix()}-delete",
            kwargs={"pk": self.id},
        )
...
class LifeInsurance(Insurace):
    ...
    insured = models.ForeignKey(
        Insured, on_delete=models.SET_NULL, blank=True, null=True
    )
    beneficiary = models.ForeignKey(
        Beneficiary, on_delete=models.SET_NULL, blank=True, null=True
    )
